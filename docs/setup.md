...
### オープンプロキシ乱用対策（中継先スキーム・ポート / 同時接続数・任意）

認証なしオープンプロキシの踏み台・資源枯渇を緩和するサーバー専用 env（`NEXT_PUBLIC_` なし）。仕様は [docs/spec/features/proxy.md §オープンプロキシ乱用対策](spec/features/proxy.md#オープンプロキシ乱用対策133)。

| 環境変数                      | 既定値 | 用途                                                                                                                                                                      |
| ----------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `PROXY_ALLOWED_PORTS`         | （空） | 中継先に許可する**追加ポート**をカンマ区切りで列挙（例 `8080,8443`）。既定の 80 / 443 は常に許可され、これに追加する。`1`〜`65535` の整数以外は無視。許可外ポートは `403` |
| `PROXY_MAX_CONCURRENT`        | `512`  | **グローバル**同時処理数の上限。超過時は `503` を返す。正の整数以外・未設定は既定値。インスタンスの処理能力に合わせて調整                                                 |
| `PROXY_MAX_CONCURRENT_PER_IP` | `64`   | **IP 単位**の同時処理数の上限。超過時は `429` を返す。1 ページが並列中継するアセット数を吸収できる値。正の整数以外・未設定は既定値                                        |

> **計上範囲**: 同時接続スロットは上流取得〜レスポンス構築までを計上し、ストリーム透過アセットの本文転送中は計上しません。中継先スキームは `http` / `https` のみ許可します（許可外は `403`）。詳細・既知の制約は上記仕様を参照。

### 認証 / 接続元許可制（共有トークン・任意）

オープンプロキシを**制限環境で運用したい場合のみ**有効化する共有トークン認証のサーバー専用 env（`NEXT_PUBLIC_` なし）。**未設定なら従来どおり認証なし（オープン）**で後方互換。仕様は [docs/spec/features/proxy.md §認証 / 接続元許可制](spec/features/proxy.md#認証--接続元許可制任意148)。

| 環境変数           | 既定値 | 用途                                                                                                                                                                                                                           |
| ------------------ | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `PROXY_AUTH_TOKEN` | （空） | 設定すると全中継経路（`/browse`・`/api/proxy`）で共有トークン認証を要求する。未設定・空白のみは無効（オープン）。利用者は 401 解錠ページでトークンを入力（`HttpOnly` Cookie を発行）するか、`X-Proxy-Token` ヘッダーで提示する |

> **TLS 必須**: トークンは Cookie 値・`X-Proxy-Token` ヘッダーに平文で載ります。盗聴を防ぐため、有効化する場合は前段で必ず TLS 終端してください。単一の共有トークンであり利用者ごとの識別・失効はできません（詳細・既知の制約は上記仕様を参照）。
