...
- [x] リポジトリ description / topics 設定（`gh repo edit`。PR 外のリポジトリ操作）
- [x] 本 PR のマージ

### Phase 2: 公開切り替えと保護強化（完了・2026-07-19）

- [x] `gh repo edit --visibility public`（ライセンスは MIT として自動認識）
- [x] `main` の branch protection 設定（PR 必須・承認 0・`enforce_admins` 有効・force push / 削除禁止。yt-gui と同構成。[git-workflow.md](../git-workflow.md) §1 のサーバー側保護が稼働）。必須ステータスチェックは CI 導入後に追加（#208）
- [x] secret scanning + push protection 有効化
- [x] Dependabot alerts 有効化

### Phase 3: 公開後の任意強化（[#208](https://github.com/f8924919/web-proxy/issues/208) へ分割）

CI 導入（[ci-guide.md](../ci-guide.md)）・CodeQL・dependabot.yml・必須チェック昇格。CONTRIBUTING / テンプレ類は外部コントリビュータが現れてから検討。
