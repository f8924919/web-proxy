...
| ------------------------------------------------ | ------------ | ------------ | ---------------------------------------------------------------------------------------------------------------------------- |
| SSRF ブロック（事前検査・IP ピン留めのいずれも） | `403`        | `403`        | [§SSRF 対策](#ssrf-対策)。ブラウザティアで遮断された場合も中継ティアへフォールバックせず `403`                               |
| 中継本文が上限超過                               | `413`        | `413`        | [§中継本文のサイズ上限](#中継本文のサイズ上限メモリ枯渇-dos-対策134)。対象は全量バッファする HTML / CSS のみ                 |
