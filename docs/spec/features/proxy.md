...
- **運用上の前提**: **本サービスは中継サイト間を分離しない。信頼できないサイトと、認証セッションを持つサイトを同一タブ（同一プロキシ origin）で併用しないこと。** 本質的な解決（サイトごとのオリジン分離＝サブドメイン割当＋ワイルドカード証明書、または `HttpOnly` Cookie をサーバー側 jar に保持して `document.cookie` 露出をなくす設計）は方針合意のうえ別途切り出す（本 Issue のスコープ外）。
...
本サービスは認証なしで誰でも任意の `http(s)` 先を中継できる**オープンプロキシ**であり、その性質上「開かれていること」は前提とする。一方で制御が皆無だと、踏み台・匿名化・スパム・違法コンテンツ中継の中継点になり得る（攻撃先から見た送信元 IP がこのサーバーになる。OWASP A04:2021 / CWE-441）。これを緩和するため、**認証は導入せずに**次の最小対策を講じる。認証 / 接続元許可制の導入要否は方針依存のため別 Issue（[#148](https://github.com/f8924919/web-proxy/issues/148)）で扱う。
