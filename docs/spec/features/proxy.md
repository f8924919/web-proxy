...
| 同一オリジンの `/_next/*`（ターゲットが Next.js 製の場合の `static` チャンク・`data`・`image` 等）                   | ターゲット origin に解決し `/api/proxy/<scheme>/<host>/_next/...`（下記参照。#102・#178）                                                                                               |
| 自前ルート（`/browse`・`/api/proxy/*`・`/sw.js`・`/favicon.ico`・ホーム `/`）                                        | 横取りせず素通し（パス反映済みの相対 import `/api/proxy/<scheme>/<host>/<path>` もここで素通しされ、ルートが中継する。#100）                                                            |
...
- **`/_next/*` の扱い（#102・#178）**: `/_next/*` は**自前ルート扱いにせず**、上記のルート絶対パスと同じ一般規則（要求元ページからターゲット origin を復元して `/api/proxy` へ振り向け）に委ねる。プロキシ対象が Next.js 製サイトの場合、クライアントランタイムは `/_next/image?url=<外部>`（`<Image>` の srcset 再生成。#102）に加え、`/_next/static/*` の遅延チャンクや `/_next/data/<buildId>/*.json`（SPA クライアント遷移のページデータ）を**プロキシ origin 直下**へ要求する。これを素通しするとプロキシ自身の Next.js サーバーに着地し、`/_next/image` は `images.remotePatterns` 未許可で **400**、`static` / `data` は **404** となり、SPA 遷移でページが空白になる（#178。実測: react.dev）。ターゲットを特定できないページ（ホーム等、`url` パラメータ無し）では従来どおり素通しし、プロキシ自身の `/_next` 資産提供には影響しない（プロキシ経由の閲覧ページはプロキシ自身の `/_next` 資産を使わない: アドレスバー UI 等はインライン注入）。
...
これを補うため、`rewriteHtml` は `<head>` 最先頭（ページ内スクリプトより先）に **`window.fetch` ・ `XMLHttpRequest.prototype.open` ・ `navigator.sendBeacon` を上書きする横取りシム `<script>`** を注入する。SW の制御確立を待たず、リクエスト URL を SW と**同一の規則**で `/api/proxy/<scheme>/<host>/<path>` へ書き換える。

| リクエスト種別                                                                                          | シムの書き換え後                                                                                        |
| ------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| クロスオリジンの絶対 URL                                                                                | `/api/proxy/<scheme>/<host>/<path>`                                                                     |
| 同一オリジンのルート絶対パス（`/path`）                                                                 | 閲覧ページ URL からターゲット origin を復元して `/api/proxy/<scheme>/<host>/<path>`                     |
| 自前ルート（`/browse`・`/api/proxy/*`・`/sw.js`・`/favicon.ico`・ホーム `/`）・非 http(s)（`data:` 等） | 横取りせず素通し（`/_next/*` は自前ルート扱いせず、ルート絶対パスの一般規則でターゲット解決する。#178） |
...
- **描画済み DOM のサイズ上限（[#144](https://github.com/f8924919/web-proxy/issues/144)）**: `page.content()` の**直前**（`inlineCssomStyles` 実体化の後）に、ブラウザ context 内で描画済み DOM の UTF-8 バイト数を概算する。`PROXY_MAX_BUFFER_BYTES`（[§中継本文のサイズ上限](#中継本文のサイズ上限メモリ枯渇-dos-対策134)）を超える場合は `page.content()` を呼ばずに取得を打ち切り、`BodyTooLargeError` 経由で `413` を返す（中継ティアと同じ上限・同じ応答）。`page.content()` で Node ヒープへ全量転送する前にブラウザ側で測ることで、展開後にしか効かない `readTextWithLimit` では防げないメモリ常駐を未然に断つ。概算ベースである点と限界は [§中継本文のサイズ上限](#中継本文のサイズ上限メモリ枯渇-dos-対策134) を参照。自動昇格ブロックでの扱いは [§ヒューリスティック自動ティア昇格](#ヒューリスティック自動ティア昇格崩れチャレンジ検出) を参照（#253）。
...
- **誤検知時の影響最小化（best-effort）**: **昇格の処理が途中のどの段階で失敗しても**、初回の中継ティア応答を**そのまま返す**（昇格は best-effort で全損にしない）。SSRF は初回ナビゲーション URL で既に検査済み。
  - 対象は `browserFetch` 自体の失敗だけでなく、**取得は成功したが後続の本文読み取りが失敗した場合**も含む。応答は本文・ステータス・ヘッダー・書き換え基準 URL の**すべてが中継ティアのもので一貫する**（一部だけブラウザティアのものが混ざることはない・[#253](https://github.com/f8924919/web-proxy/issues/253)）。
  - **昇格後の本文が上限超過（`413` 相当）でも中継ティアへ巻き戻す**。中継ティアの本文は既に読み終えており再取得が発生しないため、[§中継本文のサイズ上限](#中継本文のサイズ上限メモリ枯渇-dos-対策134)の 413 は適用しない。任意機能である自動昇格を有効にしただけで、それまで見えていたページが見えなくなる退行を避ける。
  - **Cookie は巻き戻さない**。`browserFetch` 自体は成功しており、取得した Cookie はターゲットが正当に発行したものなので、[§Cookie セッションウォーミング](#cookie-セッションウォーミング)として保持する。
  - **再昇格抑止は巻き戻さない**。失敗した昇格も抑止ウィンドウを消費する（同じ失敗が繰り返される URL でブラウザティアを起動し直すコストを避けるため）。
  - **昇格中の SSRF 遮断も巻き戻す**。`browserFetch` は昇格時にも SSRF 検査を行うため DNS の変化で遮断されうるが、返す中継ティアの本文は検査済みの経路で取得したものなので、[§SSRF 対策](#ssrf-対策)の「ブロック対象は 403」に対する**明示的な例外**として中継ティアの応答を返す。
...
| 抑止ウィンドウ | 60 秒                                                                                           |
| 昇格失敗時     | 初回の中継ティア応答へフォールバック（best-effort）。取得後の本文読み取り失敗・上限超過も含む   |
...
上流の `Link:` レスポンスヘッダー（RFC 8288）を素通しすると、`Link: <https://cdn.example/...>; rel=preload; as=style` を含むサイト（実測: qiita.com）で**ブラウザがヘッダー起点で素の URL を直接プリロード**する。この経路は DOM 書き換え・ランタイムシムが介在できず、初回ロードの SW ギャップ中はプロキシ離脱（同一リソースの二重ロード）になる。そこで `sanitizeHeaders` で `Link` ヘッダーをエントリ単位で処理する。

| `rel`                                    | 処理                                                                                                                       |
| ---------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| `preload` / `prefetch` / `modulepreload` | `<URL>` をターゲット URL 基準で解決し、パス反映形式 `/api/proxy/<scheme>/<host>/<path>` へ書き換えて維持（性能便益を保つ） |
| `preconnect` / `dns-prefetch`            | エントリを削除（接続先はプロキシ自身になるためヒントとして意味を成さず、素の URL を残すと離脱情報になる）                  |
| その他（`canonical` / `alternate` 等）   | フェッチを誘発しない情報系のためそのまま維持（HTML の `<link>` 書き換えの非リソース rel 素通しと整合）                     |

- **解析の安全側**: `<URL>` の解決が http(s) にならないエントリ・`rel` 不明のフェッチ系・値全体が解析不能な場合は、**当該エントリ（またはヘッダーごと）削除**する（素の URL を残さない fail-closed）。
- **区切りの扱い**: エントリ区切りのカンマは `<...>` および quoted-string 内のカンマと区別して分割する。
- **適用範囲**: `sanitizeHeaders` を通る中継レスポンス（`/browse`・`/api/proxy`）が対象。ターゲット URL を渡せない呼び出しでは `Link` を除去する（安全側デフォルト）。
...
**上流への中継そのものが失敗したとき**に返すステータスの正本。`/browse`（ブラウズ中継）と `/api/proxy`（アセット中継）で外形的な写像は一致する。中継に至る前に弾く場合（URL 不正の `400`、未認証の `401`、[§中継対象スキーム・ポートの制限](#中継対象スキームポートの制限133)の `403`）は本表の対象外で、各節に記す。

| 状況                                             | `/browse`    | `/api/proxy` | 備考                                                                                                                                                                               |
| ------------------------------------------------ | ------------ | ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| SSRF ブロック（事前検査・IP ピン留めのいずれも） | `403`        | `403`        | [§SSRF 対策](#ssrf-対策)。ブラウザティアで遮断された場合も中継ティアへフォールバックせず `403`。**自動昇格ブロック内での再取得は例外**（中継ティアへ巻き戻す・#253）               |
| 中継本文が上限超過                               | `413`        | `413`        | [§中継本文のサイズ上限](#中継本文のサイズ上限メモリ枯渇-dos-対策134)。対象は全量バッファする HTML / CSS のみ。**自動昇格ブロック内での再取得は例外**（中継ティアへ巻き戻す・#253） |
| その他の予期しない例外                           | `502`        | `502`        | ハンドラをクラッシュ（500）させない                                                                                                                                                |
| リダイレクト追従の上限超過                       | `502`        | `502`        | [§リダイレクト追従](#リダイレクト追従)                                                                                                                                             |
| タイムアウト / 到達不能                          | `502`        | `502`        | 上流失敗の原因は応答では区別せずログに残す（[§エラーログとプライバシー](#エラーログとプライバシー機微-url-のマスキング138)）                                                       |
| 同時接続数の上限超過                             | `503`／`429` | `503`／`429` | グローバル上限は `503`、IP 単位上限は `429`（[§同時接続数の制限](#同時接続数の制限133)）                                                                                           |
| レート制限超過                                   | `429`        | `429`        | —                                                                                                                                                                                  |

実装上は**上流取得の失敗**と**本文展開の失敗**で catch が分かれており、上流取得の失敗を分類する粒度も `/browse` と `/api/proxy` で異なる（`/api/proxy` は SSRF 以外を一律 `502` に丸める）。ただし本文上限超過は本文展開側の catch が両者とも `413` を返すため、**外形的な写像は上表のとおり揃う**。詳細は [arch/proxy.md §エラー型](../../arch/proxy.md#エラー型) を参照。

> 実装上の対応は [arch/proxy.md §エラー型](../../arch/proxy.md#エラー型) を参照。**SSRF 遮断・本文上限超過・上流失敗（502 系）の行は自動テストで固定している**（同 §ステータス写像のテスト）。同時接続数・レート制限の行は、上限判定そのものは `concurrency.ts` / `rateLimit.ts` の単体テストで検証しているが、relay 層での**ステータスへの写像は未固定**。
...
ブロック対象の場合は HTTP 403 を返す。**リダイレクト追従時は追従先 URL にも毎ホップ同じチェックを適用する**（[§リダイレクト追従](#リダイレクト追従)）。事前検査で弾かれた場合も IP ピン留め（次節）で弾かれた場合も同じ 403 を返す（[§中継失敗時のステータス](#中継失敗時のステータス)）。
...
- **上流 fetch 失敗の原因を残す**（[#236](https://github.com/f8924919/web-proxy/issues/236)）: 上流への接続が失敗したときは、ユーザーへ返す応答（502）は変えずに、**原因例外を含めて**ログする。原因は `cause` 連鎖に多段で入り得る（例: `TypeError: fetch failed` → ランタイム側の実エラー）ため、連鎖を上限付きで辿って出力する。redact 方針は上記のまま適用する。
