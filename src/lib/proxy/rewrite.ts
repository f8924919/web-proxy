...
function browseUrl(href: string, base: string): string {
  return `/browse?url=${encodeURIComponent(resolve(href, base))}`;
}

function assetUrl(href: string, base: string): string {
  return `/api/proxy?url=${encodeURIComponent(resolve(href, base))}`;
}
...
<div id="proxy-addressbar" style="position:sticky;top:0;z-index:99999;background:#1e1e2e;padding:6px 12px;display:flex;gap:8px;align-items:center;font-family:sans-serif;box-shadow:0 2px 4px rgba(0,0,0,.4)">
  <form onsubmit="(function(e){e.preventDefault();var v=e.target.querySelector('input').value;if(!v)return;window.location.href='/browse?url='+encodeURIComponent(v.startsWith('http')?v:'https://'+v)})(event)" style="display:flex;flex:1;gap:8px">
  ...
  <a href="/" style="color:#aaa;font-size:13px;text-decoration:none">ホーム</a>
</div>`.trim();
