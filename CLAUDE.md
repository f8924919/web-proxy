...
- **`main` で直接作業しない**: 必ず `main` からブランチを切り、`main` へ PR を出す（GitHub Flow）。`main` 上での**ファイル編集**と `git commit` / `git push` は hook がブロックする（[docs/git-workflow.md](docs/git-workflow.md) §1・§5.7）。
